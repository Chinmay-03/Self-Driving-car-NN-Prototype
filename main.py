...
def draw_win(cars, road, world, GEN):  # x and y are the coordinates of the best car
    ...
    py.display.update()
    world.win.blit(bg, (0,
                        0))  # blit of the background immediately after the update so if I have draws before draw_win they are not covered by the background
    ...
    while run:
        ...
        while (i < len(cars)):
            ...
            if t > 10 and (car.detectCollision(road) or y > world.getBestCarPos()[
                1] + BAD_GENOME_TRESHOLD or y > y_old or car.vel < 0.1):  # the t is used to avoid eliminating machines in the first few frames (in the first frames getCollision () always returns true
                ge[i].fitness -= 1
                cars.pop(i)
                nets.pop(i)
                ge.pop(i)
